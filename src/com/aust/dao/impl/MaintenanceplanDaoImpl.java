...
public class MaintenanceplanDaoImpl extends BaseDaoImpl implements MaintenanceplanDaoInter {

	@Override
	public List<Maintenanceplan> getMaintenanceplanList(String sql, List<Object> param) {
		// 数据集合
		List<Maintenanceplan> list = new LinkedList<>();
		try {
			// 获取数据库连接
			...
			while (rs.next()) {
				...
				for (int i = 1; i <= meta.getColumnCount(); i++) {
					...
					BeanUtils.setProperty(maintenanceplan, field, rs.getObject(field));
				}
				// 查询部门
				Device device = (Device) getObject(Device.class, "SELECT * FROM cc_device WHERE id=?",
						new Object[] { maintenanceplan.getDeviceid() });
				// 查询工种
				Employee operator = (Employee) getObject(Employee.class, "SELECT * FROM cc_employee WHERE id=?",
						new Object[] { maintenanceplan.getOperatorid() });

				// 添加
				maintenanceplan.setDevice(device);
				maintenanceplan.setOperator(operator);
				// 添加到集合
