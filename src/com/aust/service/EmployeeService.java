...
public class EmployeeService {
	...
	public Employee getEmployee(String loginname) {
		...
		Employee employee = dao.getEmployeeList("SELECT * FROM cc_employee WHERE loginname = '"+loginname+"'", null).get(0);
	...
	private void insertTransferJob(int employeeid, String oldid, String newdepartmentid, int userid) {
		...
		dao.insert("INSERT INTO cc_jobtransfer(employeeid, olddepartmentid,departmentid,transferdate,transferreson, operatorid,remark )"
				+ " value(?,?,?,?,?,?,?)", 
				new Object[]{
						employeeid,
						oldid,
	...
	public int addEmployee(Employee employee) {
		...
		//添加员工记录
				dao.insert("INSERT INTO cc_employee(name, loginname,sex,birthday,tel, address,departmentid,"
						+ "professionid,authorityid,jobstate,cardnumber,picture,remark) value(?,?,?,?,?,?,?,?,?,?,?,?,?)", 
						new Object[]{
							employee.getName(),
							employee.getLoginname(),
							employee.getSex(),
							StringTool.StrTosqlDate(employee.getBirthday()),
							employee.getTel(),
							employee.getAddress(),
							employee.getDepartmentid(),
							employee.getProfessionid(),
							employee.getAuthorityid(),
							employee.getJobstate(),
	...
	public String getStudentList(String account, Page page) {
		...
		Employee employee = (Employee) dao.getObject(Employee.class, "SELECT * FROM cc_employee WHERE loginname=?", new Object[]{account});
