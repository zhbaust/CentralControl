	  ...
	  //修改按钮
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectRow == null){
            	...
            	var authorityid = selectRow.id;
            	$.messager.confirm("消息提醒", "将删除与课程相关的所有数据，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "AuthorityServlet?method=DeleteAuthority",
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "AuthorityServlet?method=AddAuthority",
								...
								success: function(msg){
									if(msg == "success"){
										...
										$.messager.alert("消息提醒","添加权限名称已经存在!","warning");
									}else{
	    	...
	    	buttons: [
				{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "AuthorityServlet?method=editAuthority",
								data: data,
								success: function(msg){
									if(msg == "success"){
...
<body>
	...
	<div id="editDialog" style="padding: 10px">
	    	...
	    	<table id="editTable" border=0 style=" table-layout:fixed;" cellpadding="8" >
	    		<tr>
	    			<td>名称:</td>
	    			<td><input id="edit_name" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="name" data-options="required:true, missingMessage:'请填写权限名称'" /></td>
	    		</tr>
	    		<tr>
	    			<td>备注:</td>
